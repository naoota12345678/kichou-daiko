/** 電帳法 事務処理規程の自動生成 (Python regulation.py の TypeScript移植) */

const REGULATION_TEMPLATE = `電子取引データの訂正及び削除の防止に関する事務処理規程
...
（適用範囲）
第2条　本規程は、{companyName}（以下「当社」という。）における電子取引の
...
附則
本規程は、{effectiveDate}から施行する。
`;

export function generateRegulation(
  companyName: string,
  effectiveDate?: string
): string {
  const dateStr =
    effectiveDate ?? formatJpDate(new Date());

  return REGULATION_TEMPLATE
    .replace("{companyName}", companyName)
    .replace("{effectiveDate}", dateStr);
}
